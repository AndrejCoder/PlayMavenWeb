...
import ru.lanit.StudentsApplication;
import ru.lanit.dao.DaoException;
import ru.lanit.dao.GroupDao;
import ru.lanit.models.Group;
import ru.lanit.models.query.QGroup;
import ru.lanit.providers.SortableGroupDataProvider;
...
//        final StudentPanel studentPanel = new StudentPanel("studentPanel", new PropertyModel<Group>(this, "selectedGroup"));
//        add(studentPanel);

        List<Group> anyGroup = Group.find.where().ilike("name", "%енс%").findList();

        assert anyGroup != null;
        add(new Label("testEbean", Model.of(anyGroup.get(0).getName())));
...
//
//
//        List<IColumn<Group, String>> groupColumns = new ArrayList<IColumn<Group, String>>();
//
